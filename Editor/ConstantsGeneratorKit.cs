...
namespace Prime31Editor
{
	...
	public class ConstantsGeneratorKit : MonoBehaviour
	{
		...
		private static string[] IGNORE_RESOURCES_IN_SUBFOLDERS = new string[] { "ProCore", "2DToolkit", "TextMesh Pro", "Scripts", "Scene Previews", "Editor", "BetterDefines" };
		...
		private static string TOTAL_SCENES_CONSTANT_NAME = CONSTANT_NAMING_STYLE == ConstantNamingStyle.UppercaseWithUnderscores ? "TOTAL" : "Total";


		[MenuItem( "Tools/Generate Constants Classes..." )]
        ...
        public static void RebuildConstantsClass(string fileName, string[] constantsNamesArray, string totalConstantName)
        {
            ...
            AssetDatabase.ImportAsset("Assets/" + FOLDER_LOCATION + fileName, ImportAssetOptions.ForceUpdate);
				...
				public static void RebuildEnumClass(string fileName, string[] constantsNamesArray)
        {
            var folderPath = Application.dataPath + "/" + FOLDER_LOCATION_ENUMS;
            if (!Directory.Exists(folderPath))
                Directory.CreateDirectory(folderPath);

            File.WriteAllText(folderPath + fileName, GetEnumContent(fileName.Replace(".cs", string.Empty), constantsNamesArray));
            AssetDatabase.ImportAsset("Assets/" + FOLDER_LOCATION_ENUMS + fileName, ImportAssetOptions.ForceUpdate);
        }

				private static string GetEnumContent(string enumName, string[] labelsArray)
        {
            var output = "";
		...
		private static string getResourcePathsContent( string className )
		{
			...
			foreach( var res in resources )
			{
				...
				string constantName = formatVariableName( res.name );
		...
		private static string getLayerClassContent( string className, string[] labelsArray )
		{
			...
			foreach( var label in labelsArray )
				output += "\t\t" + "public const int " + formatVariableName( label ) + " = " + LayerMask.NameToLayer( label ) + ";\n";
		...
		private static string getSortingLayerClassContent( string className, string[] sortingLayers, int[] layerIds )
		{
			...
			for( var i = 0; i < sortingLayers.Length; i++ )
				output += "\t\t" + "public const int " + formatVariableName( sortingLayers[i] ) + " = " + layerIds[i] + ";\n";
        ...
        private static string buildConstIntVariable(string varName, int value)
        {
            return $"public const int {formatVariableName(varName)} = {value};";
        }

				private static string buildEnumEntry(string enumName, int enumValue)
        {
            return $"{formatVariableName(enumName)} = {enumValue},";
        }
        ...
        private static string buildConstVariable( string varName, string suffix = "", string value = null )
		{
            ...
            return "public const string " + formatVariableName( varName ) + suffix + " = " + '"' + value + '"' + ";";
		}
